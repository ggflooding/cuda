...
#define PrT (0.7)  //0.001767358!0.7

//Macroscopic properties
#define L_P (0.022)  //0.01/2/2.5/1.38/10*2.23 ! Channel height (m)
...
#define DX_P (0.092 / 500.)  //BLOK/(NY-2)	! (m)grid size
#define DT_P ((DX_P) * (UMAX) / (U_P)) //phesical time step size(s)
#define DT_PPPPP 3E-3 //phesical time step size(s)
#define alpha ((NU_P) / (Pr))   //thermal diffusivity
#define Battery_density (2580.)  //(kg/m^3)
#define Battery_capacity (830.)  //(J/K)
#define Battery_conductivity (3.4)  //(w/m K)
...
#define UMAX (0.02) //LBM U0
#define NU_LB (UMAX * (NY * 4) / (RE))  //LB viscosity
#define NU_LB1 ((NU_P)/((DX_P)*(DX_P))*DT_P)  //LB viscosity
...
#define ALFA_LB (NU_LB / Pr)
#define ALFA_LB1 (alpha / (DX_P * DX_P) * DT_P)  //NU_LB/pr
#define TauG (0.5 + (3 * ALFA_LB)) //thermal relaxation time
#define tau_solid ((Battery_conductivity / (Battery_capacity*Battery_density)) / ((DX_P) * (DX_P)) * (DT_PPPPP) * 3. + 0.5)  //solid relaxation time
#define tau_cop ((0.4 / (800. * 2900.)) / (DX_P * DX_P) * DT_PPPPP * 3. + 0.5)
...
double fx1in[720 * 8];
double fy1in[720 * 8];
double cylinderX[720 * 8];
double cylinderY[720 * 8];
double cylinderX1[720 * 8];
double cylinderY1[720 * 8];
...
__constant__ double dev_BLKRAD ((NY*4) * BLOK * 0.5 / 0.092);  //how many node in radius
__constant__ int dev_BLKCNTX[16] = {250,250,375,375,500,500,625,625,750,750,875,875,1000,1000,1125,1125};
__constant__ int dev_BLKCNTY[16] ={ 0 ,NY , 0 ,NY , 0 ,NY , 0 ,NY , 0 ,NY , 0 ,NY , 0  ,NY  , 0  ,NY  };
...
void Init();
void Load();
void Interface_coordinate();
void Point_Interface();
...
__device__ double d_CfeqT(double u[2], double T, int k);
...
__global__ void Thermal_LES(attribute *domain);
...
__global__ void HeatGeneration(attribute *domain, double *dev_SOC);
__global__ void ThermalCollision(attribute *domain);
__global__ void Streaming(attribute *domain);
__global__ void ThermalStreaming(attribute *domain);
__global__ void Inlet(attribute *domain);
__global__ void Period(attribute *domain);
__global__ void Outlet(attribute *domain);
__global__ void ThermalBC1(attribute *domain);
__global__ void ThermalBC2(attribute *domain);
__global__ void ThermalBC3(attribute *domain);
__global__ void dev_Interface_coordinate(attribute *domain, double *dev_fx, double *dev_fx1, double *dev_fy, double *dev_fy1,double *dev_fxin, double *dev_fx1in, double *dev_fyin, double *dev_fy1in);
...
__global__ void SummationThermal(attribute *domain);
__global__ void Error(attribute *domain, double *err);
__global__ void Thermal_Error(attribute *domain);
...
__global__ void dev_Interface_Temp(attribute *domain, double *dev_fx, double *dev_fx1, double *dev_fy, double *dev_fy1,double *dev_fxin, double *dev_fx1in, double *dev_fyin, double *dev_fy1in);
...
cudaError_t LBM(){
	...
	double *SOC = (double*)malloc(sizeof(double));
	...
	double *dev_fx, *dev_fx1, *dev_fy, *dev_fy1;
	double *dev_fxin, *dev_fx1in, *dev_fyin, *dev_fy1in;
...
cudaStatus = cudaMalloc((void**)&dev_ux_a, sizeof(double));
	...
	cudaStatus = cudaMalloc((void**)&dev_fx, 720 * 8 * sizeof(double));
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMalloc((void**)&dev_fx1, 720 * 8 * sizeof(double));
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMalloc((void**)&dev_fy, 720 * 8 * sizeof(double));
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMalloc((void**)&dev_fy1, 720 * 8 * sizeof(double));
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMalloc((void**)&dev_fxin, 720 * 8 * sizeof(double));
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMalloc((void**)&dev_fx1in, 720 * 8 * sizeof(double));
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMalloc((void**)&dev_fyin, 720 * 8 * sizeof(double));
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMalloc((void**)&dev_fy1in, 720 * 8 * sizeof(double));
	if (cudaStatus != cudaSuccess) goto Error;
	...
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMalloc((void**)&dev_SOC, sizeof(double));
	if (cudaStatus != cudaSuccess) goto Error;
	...
	cudaStatus = cudaMemcpy(dev_fx, fx, 720 * 8 * sizeof(double), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMemcpy(dev_fx1, fx1, 720 * 8 * sizeof(double), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMemcpy(dev_fy, fy, 720 * 8 * sizeof(double), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMemcpy(dev_fy1, fy1, 720 * 8 * sizeof(double), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMemcpy(dev_fxin, fxin, 720 * 8 * sizeof(double), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMemcpy(dev_fx1in, fx1in, 720 * 8 * sizeof(double), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMemcpy(dev_fyin, fyin, 720 * 8 * sizeof(double), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) goto Error;
	cudaStatus = cudaMemcpy(dev_fy1in, fy1in, 720 * 8 * sizeof(double), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) goto Error;
	...
	for (int t = 0;; t++){
		...
		Thermal_LES <<<NBLOCK, NTHREAD >>>(dev_domain);
		Fluid_MRT1 <<<NBLOCK, NTHREAD >>>(dev_domain);
		Fluid_MRT2 <<<NBLOCK, NTHREAD >>>(dev_domain);
		Fluid_MRT3 <<<NBLOCK, NTHREAD >>>(dev_domain);
		
		*SOC = (1. - CRate * t * DT_P / 3600.);
		cudaStatus = cudaMemcpy(dev_SOC, SOC, sizeof(double), cudaMemcpyHostToDevice);
		if (cudaStatus != cudaSuccess) goto Error;
		
		HeatGeneration <<<NBLOCK, NTHREAD >>>(dev_domain, dev_SOC);
		ThermalCollision <<<NBLOCK, NTHREAD >>>(dev_domain);
		bounceback <<<NBLOCK, NTHREAD >>>(dev_domain);
		Streaming <<<NBLOCK, NTHREAD >>> (dev_domain);
		ThermalStreaming <<<NBLOCK, NTHREAD >>> (dev_domain);
		Inlet <<<NBLOCK, NTHREAD >>> (dev_domain);
		Outlet <<<NBLOCK, NTHREAD >>> (dev_domain);
		Period <<<NBLOCK, NTHREAD >>> (dev_domain);
		ThermalBC1 <<<NBLOCK, NTHREAD >>> (dev_domain);
		ThermalBC2 <<<NBLOCK, NTHREAD >>> (dev_domain);
		ThermalBC3 <<<NBLOCK, NTHREAD >>> (dev_domain);
		dev_Interface_coordinate <<<NBLOCK, NTHREAD >>>(dev_domain, dev_fx, dev_fx1, dev_fy, dev_fy1, dev_fxin, dev_fx1in, dev_fyin, dev_fy1in);
		Summation1 <<<NBLOCK, NTHREAD >>>(dev_domain);
		//Summation2 <<<NBLOCK, NTHREAD >>>(dev_domain);
		SummationThermal <<<NBLOCK, NTHREAD >>>(dev_domain);
		Summation3 <<<NBLOCK, NTHREAD >>>(dev_domain);
		Error <<< 1 , NTHREAD >>>(dev_domain, dev_err);
		//Thermal_Error <<<NBLOCK, NTHREAD >>>(dev_domain);
		MicroToMacro <<<NBLOCK, NTHREAD >>>(dev_domain);
		//dev_Interface_Temp <<<NBLOCK, NTHREAD >>>(dev_domain, dev_fx, dev_fx1, dev_fy, dev_fy1, dev_fxin, dev_fx1in, dev_fyin, dev_fy1in);
	...
	for(int n=0;n<16;n++){											//	Block Parameters  (circle parameter) 
		for(int i=0;i<NX;i++){
			for(int j=0;j<NY;j++){
				...
				r1 = (i-BLKCNTX[n])*(i-BLKCNTX[n])+(j-BLKCNTY[n])*(j-BLKCNTY[n]);
				if(r1 <= BLKRAD*BLKRAD && r1 >= (BLKRAD-3)*(BLKRAD-3)){
					domain[Ord2(i, j, NX)].SOLIDT_Sol=2;
				}
			}
		}
	}
...
void Load(){
	...
	memset(domain, 0, NX * NY * sizeof(attribute));
	FILE *fp;
	fp = fopen("double_cylin_10000000.dat", "r");
	for (int i = 0; i < NX; i++){
		for (int j = 0; j < NY; j++){
	...
	/*for (int i = 0; i < NX; i++){
		for (int j = 0; j < NY; j++){
			...
			domain[Ord2(i, j, NX)].T = (domain[Ord2(i, j, NX)].T - 293.0)/40.0;
		}
	}*/
	return;
}
...
void Point_Interface(){
	FILE *fp50 = fopen("Point_Interface.dat", "a+");
	fprintf(fp50, "ZONE T=out\n");
	for(int J=0;J<8;J++){
		for(int I=0;I<720;I++){
			fprintf(fp50, " %.15lf ", fx[720 * J + I]);
			fprintf(fp50, " %.15lf\n", fy[720 * J + I]);
		}
	}
	fprintf(fp50, "ZONE T=out1\n");
	for(int J=0;J<8;J++){
		for(int I=0;I<720;I++){
			fprintf(fp50, " %.15lf ", fx1[720 * J + I]);
			fprintf(fp50, " %.15lf\n", fy1[720 * J + I]);
		}
	}
	fprintf(fp50, "ZONE T=in\n");
	for(int J=0;J<8;J++){
		for(int I=0;I<720;I++){
			fprintf(fp50, " %.15lf ", fxin[720 * J + I]);
			fprintf(fp50, " %.15lf\n", fyin[720 * J + I]);
		}
	}
	fprintf(fp50, "ZONE T=in1\n");
	for(int J=0;J<8;J++){
		for(int I=0;I<720;I++){
			fprintf(fp50, " %.15lf ", fx1in[720 * J + I]);
			fprintf(fp50, " %.15lf\n", fy1in[720 * J + I]);
		}
	}
	fprintf(fp50, "ZONE T=cylin\n");
	for(int J=0;J<8;J++){
		for(int I=0;I<720;I++){
			fprintf(fp50, " %.15lf ", cylinderX[720 * J + I]);
			fprintf(fp50, " %.15lf\n", cylinderY[720 * J + I]);
		}
	}
	fprintf(fp50, "ZONE T=cylin1\n");
	for(int J=0;J<8;J++){
		for(int I=0;I<720;I++){
			fprintf(fp50, " %.15lf ", cylinderX1[720 * J + I]);
			fprintf(fp50, " %.15lf\n", cylinderY1[720 * J + I]);
		}
	}
	fclose(fp50);
...
void Interface_coordinate(){
	for(int J=0;J<8;J++){							//	Block Parameters  (circle parameter)
		for(int I=0;I<720;I++){
			if (BLKRAD*cos((I+1) * pi / 720) > 0 && BLKRAD * sin((I+1)*pi/720) > 0){
				cylinderX[720 * J + I] = ((BLKRAD*cos((I+1) * pi/720)) + BLKCNTX[2 * J]);
				cylinderY[720 * J + I] = ((BLKRAD*sin((I+1) * pi/720)) + BLKCNTY[2 * J]);
				fx[720 * J + I] = ((BLKRAD*cos((I+1) * pi/720)) + BLKCNTX[2 * J] + 1.0);
				fy[720 * J + I] = ((BLKRAD*sin((I+1) * pi/720)) + BLKCNTY[2 * J] + 1.0);
				fxin[720 * J + I] = ((BLKRAD*cos((I+1) * pi/720)) + BLKCNTX[2 * J] - 1.0);
				fyin[720 * J + I] = ((BLKRAD*sin((I+1) * pi/720)) + BLKCNTY[2 * J] - 1.0);
			}
			if (BLKRAD * cos((I+1) * pi/720) < 0 && BLKRAD * sin((I+1) * pi/720) > 0){
				cylinderX[720 * J + I] = ((BLKRAD * cos((I+1) * pi/720)) + BLKCNTX[2 * J]);
				cylinderY[720 * J + I] = ((BLKRAD * sin((I+1) * pi/720)) + BLKCNTY[2 * J]);
			...
			/*if (BLKRAD*cos((I+1) * pi/720) < 0.15 * BLKRAD && BLKRAD * cos((I+1) * pi/720) > -0.15 * BLKRAD \
			&& BLKRAD * sin((I+1) * pi/720) < 0.15 * BLKRAD && BLKRAD * sin((I+1) * pi/720) > -0.15 * BLKRAD){
				fx[720 *  J + I] = ((BLKRAD * cos((I+1) * pi/720)) + BLKCNTX[2 * J]);
				fy[720 *  J + I] = ((BLKRAD * sin((I+1) * pi/720)) + BLKCNTY[2 * J]);
				fxin[720 * J + I] = ((BLKRAD * cos((I+1) * pi/720)) + BLKCNTX[2 * J]);
				fyin[720 * J + I] = ((BLKRAD * sin((I+1) * pi/720)) + BLKCNTY[2 * J]);
			}*/
		}
	}
	for(int J=0;J<8;J++){							//	Block Parameters  (circle parameter)
		for(int I=0;I<720;I++){
			if (BLKRAD * cos((720 + I + 1) * pi/720) < 0 && BLKRAD * sin((720 + I + 1) * pi / 720) < 0){
				cylinderX1[720 * J + I] = ((BLKRAD * cos((720 + I + 1) * pi/720)) + BLKCNTX[2 * J + 1]);
				cylinderY1[720 * J + I] = ((BLKRAD * sin((720 + I + 1) * pi/720)) + BLKCNTY[2 * J + 1]);
				fx1[720 * J + I] = ((BLKRAD * cos((720 + I + 1) * pi/720)) + BLKCNTX[2 * J + 1] - 1.0);
				fy1[720 * J + I] = ((BLKRAD * sin((720 + I + 1) * pi/720)) + BLKCNTY[2 * J + 1] - 1.0);
				fx1in[720 * J + I] = ((BLKRAD * cos((720 + I + 1) * pi/720)) + BLKCNTX[2 * J + 1] + 1.0);
				fy1in[720 * J + I] = ((BLKRAD * sin((720 + I + 1) * pi/720)) + BLKCNTY[2 * J + 1] + 1.0);
			}
			if (BLKRAD * cos((720 + I + 1) * pi / 720) > 0 && BLKRAD * sin((720 + I + 1) * pi / 720) < 0){
				cylinderX1[720 * J + I] = ((BLKRAD * cos((720 + I + 1) * pi/720)) + BLKCNTX[2 * J + 1]);
				cylinderY1[720 * J + I] = ((BLKRAD * sin((720 + I + 1) * pi/720)) + BLKCNTY[2 * J + 1]);
			...
			/*if (BLKRAD * cos((I + 1) * pi / 720) < 0.15 * BLKRAD && BLKRAD * cos((I + 1) * pi / 720) > -0.15 * BLKRAD \
			&& BLKRAD * sin((I + 1) * pi / 720) < 0.15 * BLKRAD && BLKRAD * sin((I + 1) * pi / 720) > -0.15 * BLKRAD){
				fx1[720 * J + I] = ((BLKRAD * cos((720 + I + 1) * pi / 720)) + BLKCNTX[2 * J + 1]);
				fy1[720 * J + I] = ((BLKRAD * sin((720 + I + 1) * pi / 720)) + BLKCNTY[2 * J + 1]);
				fx1in[720 * J + I] = ((BLKRAD * cos((720 + I + 1) * pi / 720)) + BLKCNTX[2 * J + 1]);
				fy1in[720 * J + I] = ((BLKRAD * sin((720 + I + 1) * pi / 720)) + BLKCNTY[2 * J + 1]);
			}*/
		}
	}
	/*for (int I = 0; I < 720 * 8; I++){
			printf("fx=%lf ", fx[I]);
			printf("fy=%lf ", fy[I]);
			printf("fxin=%lf ", fxin[I]);
			printf("fyin=%lf ", fyin[I]);
			printf("fx1=%lf ", fx1[I]);
			printf("fy1=%lf ", fy1[I]);
			printf("fx1in=%lf ", fx1in[I]);
			printf("fy1in=%lf\n", fy1in[I]);
	}*/
	return;
}
...
void Parameter(){
	FILE *fp0 = fopen(Inputparameter, "w");
			...
			fprintf(fp0, "tau_solid = %.15lf\n", tau_solid);
			fprintf(fp0, "tau_cop = %.15lf\n", tau_cop);
...
__device__ double d_CfeqT(double u[2], double T, int k){
	double v1 = 3. * (dev_e[k * 2] * u[0] + dev_e[k * 2 + 1] * u[1]);
	double v2 = u[0] * u[0] + u[1] * u[1];
	return (T * dev_w[k] * (1. + v1 / (C) + v1 * v1 / (2. * (C) * (C)) - v2 * 3. / (2. * (C) * (C))));
}
...
__global__ void Init_1(attribute *domain){
	...
	while (tid < NX * NY){
		...
		for (int k = 0; k < Q; k++){
			...
			(domain[tid].gIn[k]) = d_CfeqT(domain[tid].u, domain[tid].T, k);
		}
...
__global__ void Thermal_LES(attribute *domain){
	int tid = threadIdx.x + blockIdx.x * blockDim.x;
	while (tid < NX * NY){                                                                         	//LES
		(domain[tid].tauT_LB) =  ((4./3.) * (1./Pr * (TauF - 0.5) + 1./PrT * (domain[tid].tau_LB - TauF)) + 0.5);
		tid += blockDim.x * gridDim.x;
	}
	return;                                                                                           //LES end
}
...
__global__ void HeatGeneration(attribute *domain, double *dev_SOC){
	...
	domain[tid].Re1 = 0.;
	domain[tid].Re2 = 0.;
	domain[tid].heat_generation = 0.;
		if (domain[tid].B==1){
			domain[tid].Re1 = ((-112. * (*dev_SOC) * (*dev_SOC) * (*dev_SOC)  \
			- 0.203 * (*dev_SOC) * (*dev_SOC) * ((domain[tid].T) * 40. + 293.) \
			+ 0.000737 * (*dev_SOC) * ((domain[tid].T) * 40. + 293.) * ((domain[tid].T) * 40. + 293.) \
			+ 0.00000753 * (((domain[tid].T) * 40. + 293.) * ((domain[tid].T) * 40. + 293.) \
			* ((domain[tid].T) * 40. + 293.)) + 301. * (*dev_SOC) * (*dev_SOC)  \
			- 0.144 * (*dev_SOC) * ((domain[tid].T) * 40. + 293.) \
			- 0.0061 * ((domain[tid].T) * 40. + 293.) * ((domain[tid].T) * 40. + 293.) \
			- 188. * (*dev_SOC) + 1.28 * ((domain[tid].T) * 40. + 293.) + 23.6) * 0.001);
			
			domain[tid].Re2 = (-0.342 + 0.979 * (*dev_SOC) - 1.49 * (*dev_SOC)*(*dev_SOC) \
			+ 0.741 * (*dev_SOC) * (*dev_SOC) * (*dev_SOC)) * 0.001;
			
			domain[tid].heat_generation = ((Amper * Amper * domain[tid].Re1 \
			- Amper * ((domain[tid].T) * 40. + 293.) * domain[tid].Re2) / 1.654E-5);      
		}
     	tid += blockDim.x * gridDim.x;
 	}		                                                                  							  //MRT end
	return;
...
__global__ void ThermalCollision(attribute *domain){
	int tid = threadIdx.x + blockIdx.x * blockDim.x;
	while (tid < NX * NY){
		domain[tid].TauG_LB = domain[tid].tauT_LB;
		if(domain[tid].B == 1){
			domain[tid].TauG_LB = tau_solid;
		}
		/*if(domain[tid].SOLIDT_Sol == 2){
			domain[tid].TauG_LB = tau_cop;
		}*/
      	for (int k = 0; k < Q; k++){
	      		(domain[tid].gOut[k]) = (domain[tid].gIn[k] - (domain[tid].gIn[k] - d_CfeqT(domain[tid].u, domain[tid].T, k)) \
	      		/ (domain[tid].TauG_LB) + dev_w[k] * domain[tid].heat_generation / (Battery_density * Battery_capacity) \
	      		/ 40. * DT_P);
      	}
     	tid += blockDim.x * gridDim.x;
 	}		                                                                  							  //MRT end
...
__global__ void ThermalStreaming(attribute *domain){                                                         //streaming
	int x,y;
	int tid = threadIdx.x + blockIdx.x * blockDim.x;
	while (tid < NX * NY){
		d_Ord2r(tid, &x, &y, NX);
		for (int k = 0; k < Q; k++){
			...
			int yn = (y + dev_e[k * 2 + 1]);
			if ((xn >= 0) && (xn < NX) && (yn >= 0) && (yn < NY)){
			(domain[d_Ord2(xn, yn, NX)].gIn[k]) = (domain[tid].gOut[k]);
			}
		}
		tid += blockDim.x * gridDim.x;
	}
...
__global__ void Outlet(attribute *domain){
	...
	for (tid = 0; tid < NY; tid++){                                                             			//outlet    
		(domain[d_Ord2(NX-1 , tid , NX)].u[0]) = ( -1. +  1. / (p0)  * (domain[d_Ord2(NX-1 , tid , NX)].pIn[0]  \
		+ domain[d_Ord2(NX-1 , tid , NX)].pIn[2] + domain[d_Ord2(NX-1 , tid , NX)].pIn[4]  \
		+ 2. * (domain[d_Ord2(NX-1 , tid , NX)].pIn[1] + domain[d_Ord2(NX-1 , tid , NX)].pIn[5]  \
		+ domain[d_Ord2(NX-1 , tid , NX)].pIn[8])));
		
		(domain[d_Ord2(NX-1 , tid , NX)].pIn[3]) = (domain[d_Ord2(NX-1 , tid , NX)].pIn[1]  \
		- 2. / 3. * (p0) / (C) * (domain[d_Ord2(NX-1 , tid , NX)].u[0]));
		
		(domain[d_Ord2(NX-1 , tid , NX)].pIn[7]) = (domain[d_Ord2(NX-1 , tid , NX)].pIn[5]  \
		+ (domain[d_Ord2(NX-1 , tid , NX)].pIn[2]-domain[d_Ord2(NX-1 , tid , NX)].pIn[4]) / 2. \
		- ((p0) * domain[d_Ord2(NX-1 , tid , NX)].u[0] / 6. / (C)) - ((p0) * domain[d_Ord2(NX-1 , tid , NX)].u[1] / 2. / (C)));
		
		(domain[d_Ord2(NX-1, tid, NX)].pIn[6]) = (domain[d_Ord2(NX-1, tid, NX)].pIn[8]  \
		+ (domain[d_Ord2(NX-1 , tid , NX)].pIn[4]-domain[d_Ord2(NX-1 , tid , NX)].pIn[2]) / 2. \
		- ((p0) * domain[d_Ord2(NX-1 , tid , NX)].u[0] / 6. / (C)) + ((p0) * domain[d_Ord2(NX-1 , tid , NX)].u[1] / 2. / (C)));
		tid += blockDim.x * gridDim.x;
		}												                                                   //outlet end
	for (tid = 0; tid < NY; tid++){                                                             			//outlet    
...
__global__ void ThermalBC1(attribute *domain){
	...
	while (tid < NY){
		for (int k = 0; k < Q; k++){
			(domain[d_Ord2(NX-1, tid, NX)].gIn[k]) = (domain[d_Ord2(NX-2, tid, NX)].gIn[k]);
		}
		//(domain[d_Ord2(NX-1, tid, NX)].gIn[3]) = 2.0*(domain[d_Ord2(NX-2, tid, NX)].gIn[3])-(domain[d_Ord2(NX-3, tid, NX)].gIn[3]);
		//(domain[d_Ord2(NX-1, tid, NX)].gIn[6]) = 2.0*(domain[d_Ord2(NX-2, tid, NX)].gIn[6])-(domain[d_Ord2(NX-3, tid, NX)].gIn[6]);
		//(domain[d_Ord2(NX-1, tid, NX)].gIn[7]) = 2.0*(domain[d_Ord2(NX-2, tid, NX)].gIn[7])-(domain[d_Ord2(NX-3, tid, NX)].gIn[7]);
	tid += blockDim.x * gridDim.x;
	}
		return;
}
...
__global__ void ThermalBC2(attribute *domain){
	...
	while (tid < NX-1){
		if (tid > 0){
			(domain[d_Ord2(tid, NY-1, NX)].gIn[4]) = (domain[d_Ord2(tid, 0, NX)].gIn[4]);
			(domain[d_Ord2(tid, NY-1, NX)].gIn[7]) = (domain[d_Ord2(tid, 0, NX)].gIn[7]);
			(domain[d_Ord2(tid, NY-1, NX)].gIn[8]) = (domain[d_Ord2(tid, 0, NX)].gIn[8]);

			(domain[d_Ord2(tid, 0, NX)].gIn[6]) = (domain[d_Ord2(tid, NY-1, NX)].gIn[6]);
			(domain[d_Ord2(tid, 0, NX)].gIn[2]) = (domain[d_Ord2(tid, NY-1, NX)].gIn[2]);
			(domain[d_Ord2(tid, 0, NX)].gIn[5]) = (domain[d_Ord2(tid, NY-1, NX)].gIn[5]);
		}
	tid += blockDim.x * gridDim.x;
	}
		return;
...
__global__ void ThermalBC3(attribute *domain){
	int tid = threadIdx.x + blockIdx.x * blockDim.x;
	while (tid < NY){
			(domain[d_Ord2(0, tid, NX)].gIn[1]) = TL * dev_w[1] + TL * dev_w[3] - (domain[d_Ord2(tid, 0, NX)].gIn[3]);
			(domain[d_Ord2(0, tid, NX)].gIn[5]) = TL * dev_w[5] + TL * dev_w[7] - (domain[d_Ord2(tid, 0, NX)].gIn[7]);
			(domain[d_Ord2(0, tid, NX)].gIn[8]) = TL * dev_w[8] + TL * dev_w[6] - (domain[d_Ord2(tid, 0, NX)].gIn[6]);
	tid += blockDim.x * gridDim.x;
	}
		return;
}
...
__global__ void dev_Interface_coordinate(attribute *domain, double *dev_fx, double *dev_fx1, double *dev_fy, double *dev_fy1,double *dev_fxin, double *dev_fx1in, double *dev_fyin, double *dev_fy1in){
	/*for (int I = 0; I < 720 * 8; I++){
			printf("dev_fx=%lf ", dev_fx[I]);
			printf("dev_fy=%lf ", dev_fy[I]);
			printf("dev_fx1=%lf ", dev_fx1[I]);
			printf("dev_fy1=%lf\n", dev_fy1[I]);
			printf("dev_fxin=%lf ", dev_fxin[I]);
			printf("dev_fyin=%lf ", dev_fyin[I]);
			printf("dev_fx1in=%lf ", dev_fx1in[I]);
			printf("dev_fy1in=%lf\n", dev_fy1in[I]);
	}*/
	int tid = threadIdx.x + blockIdx.x * blockDim.x;
	for (tid = 0; tid < 720 * 8; tid++){
			for (int k = 0; k < Q; k++){
			//double x = (domain[d_Ord2(dev_fx[tid], dev_fy[tid], NX)].gIn[k]);
			//double y = (domain[d_Ord2(dev_fxin[tid], dev_fyin[tid], NX)].gIn[2]);
			//printf("x=%lf ", x);
			//printf("y=%lf ", y);
			(domain[d_Ord2(dev_fx[tid], dev_fy[tid], NX)].gIn[k]) = (domain[d_Ord2(dev_fxin[tid], dev_fyin[tid], NX)].gIn[k]);
			(domain[d_Ord2(dev_fx1[tid], dev_fy1[tid], NX)].gIn[k]) = (domain[d_Ord2(dev_fx1in[tid], dev_fy1in[tid], NX)].gIn[k]);
		}
	tid += blockDim.x * gridDim.x;
	}
	return;
...
__global__ void dev_Interface_Temp(attribute *domain, double *dev_fx, double *dev_fx1, double *dev_fy, double *dev_fy1,double *dev_fxin, double *dev_fx1in, double *dev_fyin, double *dev_fy1in){
	...
	for (tid = 0; tid < 720 * 8; tid++){
			...
			//printf("x=%lf ", x);
			//printf("y=%lf ", y);
			(domain[d_Ord2(dev_fx[tid], dev_fy[tid], NX)].T) = (domain[d_Ord2(dev_fxin[tid], dev_fyin[tid], NX)].T);
			(domain[d_Ord2(dev_fx1[tid], dev_fy1[tid], NX)].T) = (domain[d_Ord2(dev_fx1in[tid], dev_fy1in[tid], NX)].T);
	tid += blockDim.x * gridDim.x;
	}
	return;
}
